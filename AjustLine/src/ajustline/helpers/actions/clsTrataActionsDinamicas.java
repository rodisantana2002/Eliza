...
import ajustline.helpers.mensagens.clsPSR;
...
public class clsTrataActionsDinamicas extends AbstractAction {
    ...
    public void executarMetodoDinamico() {
        ...
        catch(IllegalAccessException ex){clsPSR.prt("Não foi possível executar a operação " + methodName);}
        catch(NoSuchMethodException ex){clsPSR.prt("Não foi possível executar a operação " + methodName);}
        catch(IllegalArgumentException ex){clsPSR.prt("Não foi possível executar a operação " + methodName);}
        catch(InvocationTargetException ex){clsPSR.prt("Não foi possível executar a operação " + methodName);}
        catch(Exception ex){clsPSR.prt("Não foi possível executar a operação " + methodName);}
    }
